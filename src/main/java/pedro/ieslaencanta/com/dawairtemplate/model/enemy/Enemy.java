...
public class Enemy extends AEnemy implements ICollision {
    ...
    public void init(Coordenada p, Rectangle Board) {
        this.img = new Image(getClass().getResourceAsStream("/" + "enemigos/e1.png"));     
        this.inc=3;
        this.setSize(new Size(64,26));
        this.setPosicion(p);
        this.board=Board;
    ...
    public void shoot() {
        ...
        tempo.init(new Coordenada(this.getPosicion().getX(), this.getPosicion().getY()+10), board,-5, 2);
         getBalas().add(tempo);
        }
    }
